...
- **2026-07-08:** Added monthly snapshot scaffolding (repo-side only — Fabric notebook/pipeline still pending) — `nb_Snapshot_MDInvoices_NoFreight` reference script, `Fact_MDInvoices_NoFreight_Snapshot` semantic model table. So the open MD freight backlog will have point-in-time history once deployed. No trend page yet — see CLAUDE.md "Monthly Snapshot" section.
