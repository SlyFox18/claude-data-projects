-- ================================================================================
-- DROP AND RECREATE VIEW: vw_Fact_Parts_Open_Tickets
-- FIX: Added TOP 1 to all subqueries to handle duplicate records
-- ================================================================================
...
SELECT 
  ...
  -- Work Order Creation Date Lookup (MIN already ensures single value)
  ...
  -- Aging Sort Order (for Power BI sorting)
  CASE 
    WHEN insalord.OrderType = 'W' AND 
         (SELECT MIN(CAST(RepairOrderDetail.CreationDate AS DATE))
          FROM RepairOrderDetail
  ...
  CASE 
    ...
    THEN (SUM(insalpar.BackorderQty) * 100.0) / SUM(insalpar.OrderQty)
    ELSE 0
