# CU Usage Report

**Generated:** 2026-06-10 08:01:39
**Time Period:** Last 24 hours

---

## Summary

| Metric | Value |
|--------|-------|
| Total CU Consumed | 1510.7 CU |
| Operations | 137 |
| Avg per Operation | 11 CU |
| Peak Operation | 69.6 CU |
| F4 Capacity Used | 65.6% |

## Top CU Consumers

| Dataflow | Total CU | Avg CU | Runs |
|----------|----------|--------|------|
| df_JDIS_PART_INFORMATION_Raw | 131.5 | 32.9 | 4 |
| df_Fact_Service_Invoices | 128.8 | 42.9 | 3 |
| df_Fact_Inventory | 49.2 | 24.6 | 2 |
| df_Fact_Transfers | 49.2 | 24.6 | 2 |
| df_InTrans_PartsCounter_Raw | 43.2 | 21.6 | 2 |
| df_Invoice_Raw | 43.2 | 21.6 | 2 |
| df_GlTrans_Raw | 40.8 | 20.4 | 2 |
| df_Fact_LaborJobSummary | 35.2 | 17.6 | 2 |
| df_Fact_Invoice_UniqueCustomers | 33.2 | 16.6 | 2 |
| df_Fact_First_Pass_Fill | 33.2 | 16.6 | 2 |

## Recommendations

- WARNING: Using 65.6% of F4 capacity - monitor closely
- Consider spreading refreshes: 137 operations at hour 8

---

**Note:** CU estimates are based on refresh duration and category. Actual CU consumption may vary based on query complexity and data volume.

