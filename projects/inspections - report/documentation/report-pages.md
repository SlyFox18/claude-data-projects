...
### Trend View (added 2026-07, revised 2026-07-20)

The Details page now defaults to a **rolling 12-month trend view** on open, layered on top of the existing Jobcode/Branch matrix toggle. It was requested by Casey to give a quick "where are we headed" read before drilling into the pivot tables. Originally built as a rolling 24-month view; narrowed to rolling 12 months on 2026-07-20 per Brian/Casey's request, alongside a round of measure bug fixes (see below).

**Visuals:**
- **Combined trend chart** (line chart, visual `935cc195e3ccbf08509c`) — now plots **4 series** by `dim_DateTable[MonthYear]`, filtered to `dim_DateTable[IsRolling12Months] = True`: `Parts $ Total (Filtered)` and `WO List - Total Labor` (current period, solid lines), plus `Parts $ Total Fixed LY` and `WO List - Total Labor LY` (prior-year, dashed styling) for year-over-year comparison. `MonthYear` has `sortByColumn: SortableMonthYear` set at the model/column level (not just a visual-level sort), so it sorts chronologically everywhere it's used in the model, not only on this chart.
- **Two-stat card** (modern Card visual, `cardVisual` type, visual `976f6918c97268c9b9a1`) — shows `Avg Parts $ / Inspection (Rolling 12)` and `Avg Labor $ / Inspection (Rolling 12)` side by side as two stat callouts, each now carrying a secondary **reference label** (a feature of the modern Card visual) showing last year's same-period average via `Avg Parts $ / Inspection (Rolling 12) LY` and `Avg Labor $ / Inspection (Rolling 12) LY`.
...
**Bug fixes (2026-07-20):** A round of testing against this view surfaced three real measure bugs, all now fixed (full DAX in `dax/dax-measures-library.md` under [Trend](dax/dax-measures-library.md#trend)):
1. `Parts $ Total (Filtered)` was double-restricting to a month via two separate date relationships (labor table's creation date *and* parts table's transaction date), silently dropping most parts on any month-filtered view — e.g. Feb 2025 showed $209K instead of the correct $746K (matching the Details matrix). Fixed by adding `ALL(dim_DateTable)` to the invoice-qualifying VAR so only the parts' own transaction date governs month membership.
2. `Avg Parts $ /Inspection` went blank at the grand total under partial (not just full) slicer exclusions, due to `ISFILTERED()`-based branching. Fixed by always dividing the already context-safe `WO List - Total Parts`.
3. **Naming trap:** a *separate, similarly-named* measure, `Avg Parts $ by Job Code`, is the one actually bound to the Jobcode matrix's visible "Avg Parts $ /Inspection" column (its `displayName`/`nativeQueryRef` reads "Avg Parts $ /Inspection", but the bound measure is `Avg Parts $ by Job Code`). It had the same `SELECTEDVALUE`-based blank-at-total bug, unconditionally (no branching at all — broken at any multi-category total). Fixing #2 did not fix the Jobcode matrix's Total row for this reason; fixing this measure did, and also silently fixed `Avg Labor & Parts by Job Code`'s Total row, which had been masking the same bug (DAX treats `BLANK() + number` as just the number, so the missing Parts component wasn't visually obvious). **Takeaway:** if a similarly-named "Avg Parts" measure looks broken again, check which measure is *actually* bound to the visual — the display name and the bound measure name can differ.

Separately (a design choice, not a bug fix): the chart/card's Labor definition was switched from `Labor $$ (Filtered)` to `WO List - Total Labor` to match the Details matrix's own Labor definition; `Labor $$ (Filtered)` was deleted as no longer used.

**Known bookmark quirk (harmless, not fixed):** all four bookmarks above (`Matrix - Jobcode`, `Matrix - Branch`, `Show Trend Chart`, `Hide Trend Chart`) still carry some stale captured filter state predating the 2026-07-20 changes — leftover `IsRolling24Months` filter entries and a leftover reference to the deleted `Labor $$ (Filtered)` measure, alongside the current correct state. Verified via live click-through with no functional impact: `IsRolling12Months` is a strict subset of `IsRolling24Months`, so the redundant wider filter doesn't change what's shown, and the reference to a deleted measure appears to no-op on bookmark restore rather than error. Left as-is intentionally rather than requiring further bookmark surgery — don't mistake this for an active bug in a future session.
