...
--
-- 4. ⭐ CRITICAL FIX: Corrected aging calculation fallback logic
--    - OLD (wrong): Used separate CASE branches for Work Orders with NULL RepairOrderDetail dates
--    - NEW (correct): Uses ISNULL fallback WITHIN the Work Order date selection
--    - Work Orders without RepairOrderDetail records now fall back to Created_On, then Order_Date
--    - This matches the old report's aging logic exactly
--    - Added Aging_Base_Date column for visibility into which date is being used
--
-- ================================================================================
-- WHAT THIS FIXES:
-- ================================================================================
-- The aging bucket discrepancies were caused by Work Orders being aged from different
-- dates depending on whether RepairOrderDetail.CreationDate was NULL:
--
-- OLD BEHAVIOR (correct):
--   Work Order #12345 with NULL RepairOrderDetail.CreationDate
--   → Uses Created_On (2024-12-15) → Ages from Dec 15 → Bucket: 0-7 days
--
-- PREVIOUS NEW BEHAVIOR (wrong):
--   Work Order #12345 with NULL RepairOrderDetail.CreationDate
--   → Falls to second CASE branch → Uses CreatedDate separately → Different bucket
--
-- FIXED NEW BEHAVIOR (correct):
--   Work Order #12345 with NULL RepairOrderDetail.CreationDate
--   → ISNULL handles it within Work Order branch → Uses Created_On (2024-12-15) → Bucket: 0-7 days
--
-- ================================================================================
