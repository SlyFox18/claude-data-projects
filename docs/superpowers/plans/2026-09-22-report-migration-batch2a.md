...
## Real corrections found when Brian first refreshed all 3 reports in Desktop

**Pin Capture failed:** `The column 'IsPreviousMonth' of the table wasn't found - dim_DateTable.` Root cause: `IsPreviousMonth` and `IsRolling12Months` were deliberately dropped from the Gold-layer `dim_DateTable` build on 2026-09-09 — they baked in `DateTime.LocalNow()` as a static value at refresh time, the exact UTC-not-local bug already fixed elsewhere in this project (documented directly in `Build_Gold_DateTable.Notebook`'s own header, which explicitly says this becomes "report-layer DAX work when each report actually migrates onto this backend"). Task 2's audit correctly found these columns were genuinely used by real DAX — the gap was in this plan not anticipating that "genuinely used" doesn't always mean "still exists in Gold." Fixed by implementing both as real DAX calculated columns directly in Pin Capture's `dim_DateTable.tmdl`, sourced from the report's own already-correct `'Data Refresh'[Date]` (not `TODAY()`/`NOW()` directly, to avoid reintroducing the same bug in DAX form), with logic matching the original production Power Query exactly — see commit `3520249e` in `fabric-workspace-docs`. Real TMDL syntax note hit along the way: a calculated column's DAX goes inline on the `column Name = <dax>` declaration line itself (same pattern as `measure`/`partition`), not as a separate nested `expression` property — the latter is a real TMDL parse error caught by this repo's own `validate-tmdl.sh` hook.

**Open Work Orders failed:** `Fact_OpenWorkOrders - The column 'WorkOrder' of the table wasn't found.` Root cause, confirmed via direct schema check: `RepairOrderDetail`'s real `DP_Presentation` shortcut column is `RONumber`, not `WorkOrder` — a real rename from its own earlier Category C raw-sources migration, unrelated to this batch, that neither Task 1's audit nor the plan's own investigation caught (the audit checked column *usage*, not whether the underlying shortcut's schema had silently drifted from the old production name). Confirmed `Silver_WkRoFile` and `Silver_TechnicianPunchedDetail` both still use `WorkOrder` — the rename is isolated to `RepairOrderDetail` only, so only that source needed a fix, not every `WorkOrder` reference in the query. Also found and fixed proactively (would have failed next, same root cause): `LaborSale`/`PartSale`/`SubletSale`/`OtherSale`/`TotalSale` vs. the query's expected `LaborRevenue`/`PartsRevenue`/`SubletRevenue`/`OtherRevenue`/`TotalRevenue`. Fixed both with a single `Table.RenameColumns` right after reading `RepairOrderDetail`, so every downstream join/reference/select kept working unchanged — see commit `39701f9d`. Verified via a full automated cross-check of every bracket-referenced column in the query against all 3 real source schemas before considering it done (caught 2 harmless false positives — `[Data]`/`[Date]` from connector-accessor syntax and a documentation comment, not real columns).
