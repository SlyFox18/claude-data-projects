...
- [x] **Step 1: Add `TradeType`, `SellPrice1`, `ListPrice` to the initial `Silver_InTrans` read**
...
- [x] **Step 2: Carry the 3 new fields through the rename step**
...
- [x] **Step 3: Insert the matrix-pricing derivation between `with_margin` and `with_sales_type`**
...
- [x] **Step 4: Add the 4 new columns to the final `select(...)`**
...
- [x] **Step 5: Update the sample/diagnostic cell's null-check to sanity-check the new columns too**
...
- [x] **Step 1: Correct the facts catalog**
...
- [x] **Step 2: Correct the notebook's own header comment**
...
- [x] **Step 2: Brian — pull the change into the workspace and run the notebook**
...
- [x] **Step 1: Confirm Price Matrix isn't open anywhere in Desktop**

- [x] **Step 2: Open Price Matrix from its current production location and publish unmodified to `RP - Dev`**
...
- [x] **Step 3: In the Fabric portal for `RP - Dev`, Source control → Commit**
...
- [x] **Step 1: Run `pbir fields list` for visual-level usage**
...
- [x] **Step 2: Grep every measure table's DAX text for column references, per table**
...
- [x] **Step 3: Cross-reference each table's real usage against its current TMDL column declarations**
...
- [x] **Step 4: Check for the `DateTime.LocalNow()` bug beyond `Fact_Part_Transactions`**
...
- [x] **Step 5: Check for raw `PartNumber` text usage (control-character exposure)**
...
- [x] **Step 2: Confirm no `LH_Master_Data` references remain**
...
- [x] **Step 1: Fix `Fact_Part_Transactions.tmdl`'s rolling-window filter**
...
- [x] **Step 2: If Task 6 Step 4 found the same pattern in `Data Refresh.tmdl`, fix it the same way**
...
- [x] **Step 1: For each table with unused columns, remove the unused `column` blocks from the TMDL AND add a matching `Table.SelectColumns` to the M query**
...
- [x] **Step 2: Confirm each trimmed table's declared column count matches its M query's real output count**
...
**Real correction #2 (the first fix attempt didn't work — Brian confirmed the identical error persisted after pulling commit `a79d5673`):** `List.Buffer` was the wrong diagnosis. It changes *when* a value is evaluated, not *what type* it is — so it couldn't have fixed a genuine type mismatch, and didn't. The actual cause, confirmed by re-examining rather than patching further: `TransactionDate` is `dateTime` (matches its own TMDL declaration and the Delta-layer timestamp type), but the cutoff value was a bare `date` (`Date.From(...)`), and this warehouse's SQL engine rejects comparing the two types directly — unrelated to folding location at all. Fixed by wrapping the cutoff in `DateTime.From(...)` so both sides of the `>=` comparison are the same type (`CutoffDate = DateTime.From(Date.AddMonths(Date.From(LocalNow), -13))`), and the now-unnecessary `List.Buffer` was removed — see commit `e5d7a5b3` in `fabric-workspace-docs`.
...
- [x] **Step 1: In the `RP - Dev` workspace's Git integration pane, sync/update to pick up Tasks 7-9's commits**

- [x] **Step 2: Open Price Matrix from `RP - Dev` in Desktop, refresh**
...
- [x] **Step 3: Visually confirm the matrix-pricing visuals**
