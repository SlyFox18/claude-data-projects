...
**Real bug found and fixed mid-Task-8 (2026-09-23):** Brian's first 3 Desktop refresh attempts each hit "duplicate value not allowed on the one side of a relationship" on `dim_JobCodes.JobCode` — a different, seemingly random `JobCode` flagged each time, across 2 different connectors (`Sql.Database` and a native Lakehouse re-import). Direct investigation (Bronze/Silver/Gold layer checks via `delta_scan`, exact/case-insensitive/trim/Unicode-NFKC-normalized comparisons, Delta log commit-history check) found the *logical* data had zero real duplicates every time — but `fab dir` on the table's physical folder revealed the real root cause: `mode("overwrite")` only updates the Delta transaction log's pointer, it does **not** delete the previous run's physical Parquet files. After `Build_Gold_JobCodes.Notebook` was re-run once during this investigation, the folder had 4 physical files (the original run's 3 partition files, still present, plus the new run's file). Fabric's SQL analytics endpoint / Lakehouse connector appears to scan the physical folder rather than strictly resolving the current snapshot via the transaction log the way `delta_scan` correctly does — so it was reading the orphaned old files too, doubling every row (Power BI's error dialog reports only the first violation it hits before aborting the load, explaining the "random different code each time" symptom).

**Fixed** by adding an explicit `VACUUM ... RETAIN 0 HOURS` after every write in `Build_Gold_JobCodes.Notebook`, plus `dropDuplicates` + `coalesce(1)` as defense-in-depth (commit `55702d91` in `fabric-workspace-docs`). Verified: table folder now has exactly 1 physical file; 575,887 rows, 575,887 distinct `JobCode` values; all 3 previously-reported duplicate codes (`7180-F478/184`, `8420/4210/MWP/R/20`, `8R310/4246/4WD/25`) now show exactly 1 row each.

**Real systemic implication, not yet acted on beyond this one table:** every Gold notebook in this entire project uses `mode("overwrite")` without a `VACUUM` step, and the daily pipeline (`Pipeline_DP_Daily_Refresh`) re-runs every registered notebook on a schedule — meaning this same latent bug is likely to surface on any already-migrated table once its own notebook has run 2+ times via the daily schedule, not just this one. Flagged to Brian as a real follow-up candidate (likely best solved with a shared post-refresh VACUUM step across the pipeline rather than editing every individual notebook), not fixed broadly as part of this task — out of scope for unblocking this specific report.
