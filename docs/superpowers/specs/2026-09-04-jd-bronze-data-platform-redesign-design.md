...
Confirmed gap: `jdis_Part_Information` is not in JD's Bronze mirror. **Corrected 2026-09-09** (was previously assumed to be sourced from a different JD system entirely — it isn't): it's an Equip DB table like everything else here, JD simply chose not to include it in their mirror, almost certainly because it changes constantly (real-time parts/inventory master data) and is a poor fit for their transfer's apparent cadence. This means it can't get a OneLake shortcut into JD's mirror the way `InTrans`/`GlTrans` did — it needs its own direct ingestion from `EquipRDB`, landing into the Staging lakehouse's bronze area, the same way `LH_Master_Data` already does it today (just built properly this time, not on the watermark-append pattern that caused the original bug, unless a fresh look at this specific table's needs says otherwise). `dim_EngagedAcres` (external CSV) and Commodity Code Groups (SharePoint CSV) are genuinely non-Equip-sourced and keep their own small, existing-style ingestion regardless. A full inventory of what else `LH_Master_Data` ingests that ISN'T Equip-sourced (and therefore needs to keep its own ingestion path) is a task for the implementation plan, not fully enumerated here.
